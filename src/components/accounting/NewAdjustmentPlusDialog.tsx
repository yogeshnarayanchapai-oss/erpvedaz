...
export function NewAdjustmentPlusDialog({ open, onOpenChange, onSwitchType }: NewAdjustmentPlusDialogProps) {
  const { data: accounts = [] } = useActiveAccounts();
  const createTransaction = useCreateTransaction();

  const [formData, setFormData] = useState({
    date: format(new Date(), 'yyyy-MM-dd'), amount: '', account_id: '', reference_no: '', note: '', consignment_ids: [] as string[],
  });

  const resetForm = () => {
    setFormData({ date: format(new Date(), 'yyyy-MM-dd'), amount: '', account_id: '', reference_no: '', note: '', consignment_ids: [] });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createTransaction.mutateAsync({
        date: formData.date, transaction_type: 'ADJUSTMENT_PLUS' as TransactionType, amount: parseFloat(formData.amount),
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <p className="text-xs text-muted-foreground">Increase account balance for adjustments/corrections</p>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-3">
          <InlineTypeSelector
            currentType={'ADJUSTMENT_PLUS' as TransactionType}
            allowedTypes={['INCOME', 'PAYMENT_IN', 'SALES_IN', 'ADJUSTMENT_PLUS' as TransactionType]}
            onSelect={(type) => { if (type !== 'ADJUSTMENT_PLUS' && onSwitchType) onSwitchType(type); }}
          />
