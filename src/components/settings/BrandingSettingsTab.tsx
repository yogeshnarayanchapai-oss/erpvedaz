...
export default function BrandingSettingsTab() {
  ...
  const handleFaviconSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type) && !file.name.endsWith('.ico')) {
      toast({ title: 'Invalid file type', description: 'Please upload a PNG, ICO, or SVG file', variant: 'destructive' });
      return;
    }

    if (file.size > 500 * 1024) {
      toast({ title: 'File too large', description: 'Please upload a favicon smaller than 500KB', variant: 'destructive' });
      return;
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: 'Branding updated', description: 'Your changes have been saved successfully' });
      ...
      toast({ title: 'Error saving branding', description: error.message || 'An error occurred', variant: 'destructive' });
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      <Card className="max-w-2xl">
        ...
        <CardContent className="space-y-6">
            ...
            <div className="flex items-start gap-6">
              <div className="flex flex-col items-center gap-2">
                <div className="w-32 h-32 border rounded-lg flex items-center justify-center bg-muted/50 overflow-hidden">
                  {currentLogoUrl ? (
                    <img src={`${currentLogoUrl}?t=${Date.now()}`} alt="Logo preview" className="max-w-full max-h-full object-contain" />
                  ) : (
                    <Image className="w-12 h-12 text-muted-foreground" />
                  )}
                </div>
                <span className="text-xs text-muted-foreground">{previewUrl ? 'New logo preview' : 'Current logo'}</span>
              </div>
              <div className="flex-1 space-y-2">
                ...
                {selectedFile && <p className="text-xs text-primary">Selected: {selectedFile.name}</p>}
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start gap-6">
              <div className="flex flex-col items-center gap-2">
                <div className="w-16 h-16 border rounded-lg flex items-center justify-center bg-muted/50 overflow-hidden">
                  {currentFaviconUrl ? (
                    <img src={`${currentFaviconUrl}?t=${Date.now()}`} alt="Favicon preview" className="w-8 h-8 object-contain" />
                  ) : (
                    <Globe className="w-6 h-6 text-muted-foreground" />
                  )}
                </div>
                <span className="text-xs text-muted-foreground">{faviconPreviewUrl ? 'New favicon' : 'Current'}</span>
              </div>
              <div className="flex-1 space-y-2">
                <input ref={faviconInputRef} type="file" accept=".png,.ico,.svg" onChange={handleFaviconSelect} className="hidden" />
                <Button variant="outline" onClick={() => faviconInputRef.current?.click()} className="w-full">
                  <Upload className="w-4 h-4 mr-2" />Upload Favicon
                </Button>
                <p className="text-xs text-muted-foreground">Accepted formats: PNG, ICO, SVG. Max size: 500KB.</p>
                {selectedFavicon && <p className="text-xs text-primary">Selected: {selectedFavicon.name}</p>}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input id="brandName" value={brandName} onChange={(e) => setBrandName(e.target.value)} placeholder="Enter your brand name" maxLength={100} />
            <p className="text-xs text-muted-foreground">This name will appear in the sidebar, login pages, and throughout the system.</p>
          </div>
            ...
            <div className="flex items-center justify-between p-4 border rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">This theme will be applied across all portals by default</p>
                </div>
              </div>
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground mb-3">Select a primary color for buttons, links, and accents.</p>
              ...
              {PRESET_COLORS.map((color) => (
                <button
                  ...
                  className={`relative w-full aspect-square rounded-lg border-2 transition-all hover:scale-105 ${
                    primaryColor === color.value ? 'border-foreground ring-2 ring-offset-2 ring-offset-background' : 'border-transparent'
                  }`}
                  style={{ backgroundColor: `hsl(${color.value})` }}
                  title={color.name}
                >
                  {primaryColor === color.value && <Check className="absolute inset-0 m-auto w-5 h-5 text-white drop-shadow-md" />}
                </button>
              ))}
            </div>
            <p className="text-xs text-muted-foreground mt-2">Selected: {PRESET_COLORS.find(c => c.value === primaryColor)?.name || 'Custom'}</p>
          </div>

          {/* Custom CSS Section */}
          <div className="space-y-2">
            <Label className="flex items-center gap-2"><Code className="w-4 h-4" />Custom CSS</Label>
            <Textarea value={customCss} onChange={(e) => setCustomCss(e.target.value)} placeholder={`/* Add custom CSS here */`} className="font-mono text-sm min-h-[150px]" />
            <p className="text-xs text-muted-foreground">Add custom CSS that will be injected across all portals. Use with caution.</p>
          </div>
      ...
      <Card className="max-w-2xl">
        <CardHeader>
          ...
          <CardDescription>How your branding will appear across portals</CardDescription>
        </CardHeader>
        <CardContent>
          <div className={`flex items-center gap-3 p-4 rounded-lg border ${darkMode ? 'bg-zinc-900 text-white' : 'bg-sidebar'}`}>
            <div className="w-10 h-10 flex items-center justify-center overflow-hidden">
              {currentLogoUrl ? (
                <img src={`${currentLogoUrl}?t=${Date.now()}`} alt="Logo" className="max-w-full max-h-full object-contain" />
              ) : (
                <div className="w-10 h-10 bg-primary/20 rounded flex items-center justify-center">
                  <span className="text-primary font-bold text-lg">{brandName.charAt(0).toUpperCase() || 'V'}</span>
                </div>
              )}
            </div>
            <span className={`font-semibold ${darkMode ? 'text-white' : 'text-sidebar-foreground'}`}>{brandName || 'Zivkart OS'}</span>
          </div>
        </CardContent>
      </Card>
    </div>
