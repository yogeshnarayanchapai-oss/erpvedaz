-- Fix column name: products has 'sell_price' not 'price'
...
BEGIN
  ...
  SELECT o.id, o.product_id, o.sales_person_id, o.store_id, o.created_at, o.amount as order_total
  INTO v_last_order
  FROM orders o
  ...
  IF v_last_order.product_id IS NOT NULL THEN
    SELECT p.name, p.sell_price INTO v_product_name, v_product_price
    FROM products p
