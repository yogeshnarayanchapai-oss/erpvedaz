-- Update the get_customer_insight function to include last_order_status
...
BEGIN
  ...
  LEFT JOIN LATERAL (
    SELECT o.created_by_user_id, o.total_amount, o.order_status
    FROM orders o 
    WHERE o.customer_phone = c.phone_number 
    ORDER BY o.created_at DESC 
    LIMIT 1
  ) lo ON true
  LEFT JOIN profiles p ON p.id = lo.created_by_user_id
  LEFT JOIN LATERAL (
    SELECT oi.product_id
    FROM order_items oi
    JOIN orders o2 ON o2.id = oi.order_id
    WHERE o2.customer_phone = c.phone_number
