-- Fix the trigger to use correct column names and handle related_to_accounting toggle

-- Drop existing duplicate triggers (there are 2 doing the same thing)
DROP TRIGGER IF EXISTS create_party_tx_from_stock ON public.stock_movements;
DROP TRIGGER IF EXISTS trigger_create_party_transaction ON public.stock_movements;

-- Create a unified function that handles INSERT and related_to_accounting changes
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    -- If related_to_accounting changed from true to false, soft-delete the party transaction
    IF OLD.related_to_accounting = true AND NEW.related_to_accounting = false THEN
      -- Find and delete existing party transaction linked to this stock movement
  ...
  IF v_amount > 0 THEN
    INSERT INTO public.party_transactions (
      ...
      NEW.product_id,
      COALESCE(NEW.to_warehouse_id, NEW.from_warehouse_id),
      v_qty,
      v_rate,
      v_amount,
      v_direction,
...
CREATE TRIGGER create_party_tx_from_stock_insert
AFTER INSERT ON public.stock_movements
FOR EACH ROW
EXECUTE FUNCTION create_party_transaction_from_stock_movement();

-- Create trigger for UPDATE of related_to_accounting
CREATE TRIGGER create_party_tx_from_stock_accounting_toggle
AFTER UPDATE OF related_to_accounting ON public.stock_movements
FOR EACH ROW
